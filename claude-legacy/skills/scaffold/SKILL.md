...
3. **Explain intent.** Add short comments on each file, type, and function describing *what* it does and *why* it exists — similar to well-annotated pseudo-code.
...
6. **No tests, no docs.** Do not generate test files or documentation files unless the user explicitly asks.
...
3. **Scaffold** — Write each file with:
...
4. **Summarize** — List the files created and what each one is for.
